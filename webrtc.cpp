...
create_receiver_entry (SoupWebsocketConnection * connection)
{
  ...
  receiver_entry->pipeline =
      ...
      STUN_SERVER " "
      VIDEO_SRC
      " pattern=snow ! videorate ! videoscale ! video/x-raw,width=64,height=64,framerate=60/1 ! videoconvert ! queue max-size-buffers=1 ! x264enc bitrate=600 speed-preset=ultrafast tune=zerolatency key-int-max=15 ! video/x-h264,profile=constrained-baseline ! queue max-size-time=100000000 ! h264parse ! "
    ...
    G_GNUC_UNUSED gpointer user_data)
{
  ...
  static boost::iostreams::mapped_file mmap("../webrtc.html", boost::iostreams::mapped_file::readonly);
  soup_buffer =
